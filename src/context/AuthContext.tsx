...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const fetchSession = async () => {
      try {
        ...
        timeoutId = setTimeout(() => {
          if (loading) {
            console.warn('Session loading timeout - forcing logout')
            forceSignOut()
          }
        ...
        console.error('Error fetching session:', error)
        setError(error.message || 'Erreur de session')
        await forceSignOut()
      } finally {
    ...
    const fetchUserProfile = async (userId: string) => {
      try {
        ...
        console.error('Error fetching profile:', error)
        setError(error.message || 'Erreur de profil utilisateur')
        await forceSignOut()
      } finally {
        setLoading(false)
      }
  ...
  const signIn = async (email: string, password: string) => {
    try {
      ...
      // Nettoyage préalable des tokens
      window.localStorage.removeItem('sb-auth-token')
      window.sessionStorage.removeItem('sb-auth-token')
