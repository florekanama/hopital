...
// //   return (
// //     <div className="space-y-6 bg-red-00">
// //       <h2 className="text-xl px-2 text-center mt-2 font-bold text-gray-800">Prendre un nouveau rendez-vous</h2>
...
// //         {selectedMedecin && (
// //           <div className="mt-4 p-4 bg-blue-0 rounded-lg">
...
// //           <div className="borde  scale-[95%] border-gray rounded-lg p-1 translate-x-[-3px]">
// //             <DayPicker
// //               mode="single"
...
// //         {/* Onglets de navigation - style similaire au premier composant */}
...
// //         {/* Contenu des onglets */}
// //         <div className="bg-white rounded-xl hadow-md overflow-hidden">
// //           <div className="p-">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              >
                ...
                {filteredMedecins.map(medecin => (
                  <option key={medecin.id} value={medecin.id}>
                    Dr. {medecin.user?.nom || 'Nom non disponible'} - {medecin.specialite?.nom || 'Spécialité non disponible'}
                  </option>
                ))}
              </select>
            </div>
          ...
          {selectedMedecin && (
            <div className="mt-6 p-6 bg-blue-50 rounded-xl border border-blue-100">
              <div className="flex items-start space-x-4">
                ...
                <div className="flex-1">
                  <h3 className="text-lg font-semibold text-gray-900">
                    Dr. {medecins.find(m => m.id === selectedMedecin)?.user?.nom || 'Nom non disponible'}
                  </h3>
                  <p className="text-blue-600 font-medium">
                    {medecins.find(m => m.id === selectedMedecin)?.specialite?.nom || 'Spécialité non disponible'}
                  </p>
                  <div className="mt-2 text-sm text-gray-600">
                    <p className="flex items-center">
                      <svg className="h-4 w-4 mr-2 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <p className="flex items-center mt-1">
                      ...
                      {medecins.find(m => m.id === selectedMedecin)?.user?.email || 'Email non disponible'}
                    </p>
                    <p className="flex items-start mt-1">
                      <svg className="h-4 w-4 mr-2 text-gray-500 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                      Langues: {medecins.find(m => m.id === selectedMedecin)?.langues?.map((l: any) => l?.langue?.nom).filter(Boolean).join(', ') || 'Non spécifié'}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="flex items-start space-x-4">
                  ...
                  <div>
                    <p className="font-medium">Dr. {medecins.find(m => m.id === selectedMedecin)?.user?.nom || 'Nom non disponible'}</p>
                    <p className="text-sm text-gray-600">{medecins.find(m => m.id === selectedMedecin)?.specialite?.nom || 'Spécialité non disponible'}</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm">
                    <span className="font-medium">Lieu:</span> {creneauxDisponibles.find(c => c.debut.toISOString() === selectedCreneau)?.lieu || 'Lieu non spécifié'}
                  </p>
                </div>
              </div>
            </div>
